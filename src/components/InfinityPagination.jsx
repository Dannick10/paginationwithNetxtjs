...
const InfinityPagination = ({date, quantityforsteps}) => {
    ...
    useEffect(() => {
        const filterPost = search.length > 0 ? 
  ...
  return (
    <section className='flex flex-col items-center justify-between gap-4 min-h-screen'>
        ...
        {actualDate &&
        actualDate.map((post) => (
            <div className="border-[.1em] border-gray-300 bg-white p-10 rounded-md w-9/12 h-56 flex flex-col justify-around ">
                {console.log(post)}
        ...
        <nav>   
            {currentPage <= pageTotal ? <>
                <button className='border-gray-800 text-gray-900 border-[.1em] p-2 px-7 rounded-xl text-sl hover:bg-gray-800 hover:text-white' onClick={handlePageFetch}>Carregar</button>
                </>:''}
        </nav>
    </section>
  )
